...
#define PN532_SPI_HOST SPI2_HOST // or VSPI_HOST
#define PN532_SPI_SPEED 1000000  // 1 MHz
...
static spi_device_handle_t pn532_spi_handle;
...
void pn532_spi_init(pn532_t *obj, uint8_t clk, uint8_t miso, uint8_t mosi, uint8_t ss)
{
    ...
    spi_bus_config_t buscfg = {
        ...
        .quadwp_io_num = -1,
        .quadhd_io_num = -1,
        .max_transfer_sz = 64 // PN532 frames are small
    };

    // Configure PN532 SPI device
    spi_device_interface_config_t devcfg = {
        .clock_speed_hz = PN532_SPI_SPEED,
        .mode = 0, // SPI Mode 0
        .spics_io_num = -1,
        .queue_size = 1,
    ...
    ret = spi_bus_initialize(PN532_SPI_HOST, &buscfg, SPI_DMA_CH_AUTO);
    if (ret != ESP_OK && ret != ESP_ERR_INVALID_STATE) {
        ESP_LOGE("PN532", "Failed to init SPI bus: %s", esp_err_to_name(ret));
        return;
    }

    ret = spi_bus_add_device(PN532_SPI_HOST, &devcfg, &pn532_spi_handle);
    if (ret != ESP_OK) {
        ESP_LOGE("PN532", "Failed to add SPI device: %s", esp_err_to_name(ret));
        return;
    }

    obj->_spi = pn532_spi_handle;
}
...
void pn532_spi_write(pn532_t *obj, const uint8_t data)
{
    spi_transaction_t t = {
        .length = 8,
        .tx_buffer = &data,
    ...
    spi_device_transmit(obj->_spi, &t);
}
